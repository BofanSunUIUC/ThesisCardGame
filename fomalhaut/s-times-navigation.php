...
/*
	...
	echo "\t<script src=\"./js/jquery.textfill.min.js\"></script>\n";
	...
	//echo "\t<p>Size: <span id = \"txtHint\"></span></p>\n";

	echo "\t<center><div id= \"txtHint\" class=\"wrap\">\n";
	}
	$query = "";
	...
	if ($session !== "" and $prev_lap !== "") {
		$back_addr .= "?session=$session&lap=$prev_lap"."&l=".$l."&w=".$w."&t=b";
	}
	if ($algo === "navigation") {
	...
	if ($selection_size > 0) {
		if ($algo === "navigation") {
			$more_addr = $base_addr . "?session=$session&lap=$next_lap&selected=$selection_size"."&l=".$l."&w=".$w."&t=b";
